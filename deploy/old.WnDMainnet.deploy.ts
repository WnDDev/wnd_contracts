...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    async function main() {
        var checkPointManager = isTestNet
            ? "0x2890bA17EfE978480615e330ecB65333b880928e"
            : "0x86E4Dc95c7FBdBf52e33D563BbDB00823894C287";

        var fxRoot = isTestNet
            ? "0x3d1d3E34f7fB6D26245E6640E1c50710eFFf15bA"
            : "0xfe5e5D361b2ad62c541bAb87C45a0B9B018389a2";

        const riftRoot = await deployContract('RiftRoot');
        const consumables = await deployContract('Consumables');

        let rootTunnel;
        if(isUnitTests) {
            rootTunnel = await deployContractNonUpgradeable('MockRootTunnel');
        } else {
            rootTunnel = await deployContractNonUpgradeable('RootTunnel', checkPointManager, fxRoot);
        }

        await unpauseContractIfNeeded('RiftRoot');
        await unpauseContractIfNeeded('Consumables');

        const gpRootAddress = await getGPRootAddress();
        const saRootAddress = await getSARootAddress();
        const oldTrainingGroundsAddress = await getOldTrainingGroundsAddress();
        const wndRootAddress = await getWnDRootAddress();

        // const gpRootAddress = "0xCD321ceBB0d631D2F74d21301B3AC76067988e9f";
        // const saRootAddress = "0xE5A69ab83D722b5779eCaD4658402c2d8422fAB6";
        // const oldTrainingGroundsAddress = "0x002299d42ee14C3705Dcb31e59611303dBC6b8cc";
        // const consumablesAddress = "0x312Cf426B79FFE25aA28329E5087b0f12Fff1d2e";
        // const rootTunnelAddress = "0x38Ec27c6F05a169e7eD03132bcA7d0cfeE93C2C5";

        await setContractsIfNeeded('RiftRoot', gpRootAddress, wndRootAddress, saRootAddress, consumables.address, rootTunnel.address, oldTrainingGroundsAddress);
        await setAdminsIfNeeded('RiftRoot', rootTunnel.address);
        await setAdminsIfNeeded('Consumables', rootTunnel.address, riftRoot.address);
        // ATTENTION - MUST ADD MANUALLY FOR MAINNET
        // await setAdminsIfNeeded('GP', rootTunnel.address, riftRoot.address);
        // await setAdminsIfNeeded('WnD', rootTunnel.address, riftRoot.address);
        // await setAdminsIfNeeded('SacrificialAlter', rootTunnel.address, riftRoot.address);

        if(!isUnitTests) {
            await setAdminsIfNeeded('RootTunnel', riftRoot.address);
        }

        await setupConsumableIdsIfNeeded();
        // await setupSacrificialAlterIdsIfNeeded();

        await setMessageHandlerIfNeeded(riftRoot.address);
    ...
    async function setMessageHandlerIfNeeded(riftRootAddress: string) {
        if(isUnitTests) {
            return;
        }
        const messageHandler = await read('RootTunnel', `messageHandler`);
        if(messageHandler === riftRootAddress) {
            return;
        }
    ...
    async function getGPRootAddress() : Promise<string> {
        if(isTestNet || isUnitTests) {
            var gpRoot = await deployContractNonUpgradeable('MockRootGP');
            return gpRoot.address;
        } else {
            return "0x38Ec27c6F05a169e7eD03132bcA7d0cfeE93C2C5";
        }
    }

    async function getWnDRootAddress() : Promise<string> {
        if(isUnitTests) {
            var wndRoot = await deployContractNonUpgradeable('MockRootWnD');
            return wndRoot.address;
        } else if(isTestNet) {
            var traits = await deployContract('Traits');
            var wndRoot = await deployContract('WnD');
            await unpauseContractIfNeeded('Traits');
            await setContractsIfNeeded('WnD', traits.address);
            await setWnDForTraitIfNeeded(wndRoot.address);
            // await loadWnDTraits('Traits');
            return wndRoot.address;
        } else {
            return "0x999e88075692bCeE3dBC07e7E64cD32f39A1D3ab";
        }
    }

    async function getSARootAddress() : Promise<string> {
        if(isTestNet || isUnitTests) {
            var saRoot = await deployContract('SacrificialAlter');
            await unpauseContractIfNeeded('SacrificialAlter');
            await setupSacrificialAlterIdsIfNeeded();
            return saRoot.address;
        } else {
            return "0xFa1A07056C48DCbA4B5E9e71aACC6aa791A93929";
        }
    }

    async function getOldTrainingGroundsAddress() : Promise<string> {
        if(isTestNet || isUnitTests) {
            var oldTrainingGrounds = await deployContractNonUpgradeable('MockOldTrainingGrounds');
            return oldTrainingGrounds.address;
        } else {
            return "0xeeA43B892D4593E7C44530dB1f75b0519Db0BD50";
        }
    }
